...
const CourseReport = () => {
  ...
  const processCsvData = (rawData) => {
    try {
      ...
      console.log('第一列表頭:', rawData[0]);

      // 步驟1: 賦值 df_raw
      const dfRaw = rawData;

      // 步驟2: 重新命名星期欄位
      // 取得第一列第 8-13 欄作為星期名稱 (索引 7-12)
      const firstRow = dfRaw[0];
      const dayNames = firstRow.slice(7, 13);
      console.log('星期名稱:', dayNames);

      // 建立欄位重新命名對應表
      const originalCols = firstRow.map((_, index) => index);
      const renamedHeaders = firstRow.map((header, index) => {
        if (index >= 7 && index <= 12) {
          // 使用實際的星期名稱，如果為空則使用預設值
          return dayNames[index - 7] || `星期${index - 6}`;
        }
      ...
      if (processedRows.length > 30) {
        ...
        const col8Value = validationRow.col_7;
        const col9Value = validationRow.col_8;
        console.log('驗證第31列第8-9欄:', { col8: col8Value, col9: col9Value });
      }
  ...
  const getColumnWidth = (index, header) => {
    ...
    if (index >= 6 && index <= 12) return 60;  // 星期時間
    if (index === 13) return 100; // 教室
